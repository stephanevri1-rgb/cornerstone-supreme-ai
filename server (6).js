...
async function downloadMedia(mediaId) {
  try {
    // Step 1: Request media URL from 360dialog (returns 302 redirect to CDN)
    // We use redirect: 'manual' to capture the Location header
    const metaRes = await fetch(`${WHATSAPP_API}/v1/media/${mediaId}`, {
      method: 'GET',
    ...
    // 360dialog returns HTTP 302 with Location header pointing to CDN
    if (metaRes.status === 302 || metaRes.status === 301) {
      downloadUrl = metaRes.headers.get('location') || metaRes.headers.get('Location');
      console.log(`[MEDIA] Got 302 redirect to CDN: ${downloadUrl ? downloadUrl.substring(0, 60) + '...' : 'MISSING'}`);
    } else if (metaRes.ok) {
      // Some API versions return JSON with a url field
      const contentType = metaRes.headers.get('content-type') || '';
      if (contentType.includes('application/json')) {
        const metaData = await metaRes.json();
        downloadUrl = metaData.url || metaData.link;
        console.log(`[MEDIA] Got JSON metadata, CDN URL: ${downloadUrl ? downloadUrl.substring(0, 60) + '...' : 'MISSING'}`);
      } else {
        // Direct binary response
        const buffer = Buffer.from(await metaRes.arrayBuffer());
        if (buffer.length > 0) {
          console.log(`[MEDIA] Downloaded ${buffer.length} bytes directly`);
          return { buffer, contentType };
        }
      }
    } else {
      console.error(`[MEDIA] Media metadata failed: ${metaRes.status} ${metaRes.statusText}`);
      throw new Error(`Media metadata failed: ${metaRes.status}`);
    }

    // Step 2: Fetch the actual file from CDN (no auth needed)
    if (downloadUrl) {
      const fileRes = await fetch(downloadUrl, { method: 'GET' });
      if (!fileRes.ok) throw new Error(`CDN download failed: ${fileRes.status}`);
      const fileType = fileRes.headers.get('content-type') || 'application/octet-stream';
      const buffer = Buffer.from(await fileRes.arrayBuffer());
      console.log(`[MEDIA] Downloaded ${buffer.length} bytes from CDN, type: ${fileType}`);
