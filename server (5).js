...
async function downloadMedia(mediaId) {
  try {
    // Step 1: Get media metadata (contains the download URL)
    ...
    if (!metaRes.ok) {
      console.error(`Media metadata failed: ${metaRes.status} ${metaRes.statusText}`);
      throw new Error(`Media metadata failed: ${metaRes.status}`);
    }

    // 360dialog returns JSON with a "url" field for the actual CDN location
    ...
    if (contentType.includes('application/json')) {
      ...
      console.log(`[MEDIA] Got metadata, CDN URL: ${downloadUrl ? downloadUrl.substring(0, 60) + '...' : 'MISSING'}`);
    }

    // If we got a URL, fetch the actual binary content from CDN
    if (downloadUrl) {
      const fileRes = await fetch(downloadUrl, { method: 'GET' });
      if (!fileRes.ok) throw new Error(`CDN download failed: ${fileRes.status}`);
      const fileType = fileRes.headers.get('content-type') || contentType;
      const buffer = Buffer.from(await fileRes.arrayBuffer());
      console.log(`[MEDIA] Downloaded ${buffer.length} bytes, type: ${fileType}`);
      return { buffer, contentType: fileType };
    }

    // Fallback: try to get binary directly (some API versions return the file directly)
