...
1. **If you already know their name** → Use it. Never ask "What is your name?" again.
...
"Hi [Name], great to hear from you again! Last time we were looking at the RE 5 programme — you were interested in the online option. Have you had a chance to think about it? I can send you the enrolment link whenever you're ready."
...
**Example of NEVER asking known info:**
If the Prospect Profile shows "Course Interest: RE 5" and "Full Name: John Smith", you would NEVER say "Which course are you interested in?" or "What is your name?". Instead: "Hi John! How are you feeling about the RE 5 programme? Do you have any questions before we get you registered?"
...
async function downloadMedia(mediaId) {
  try {
    // Step 1: Get media metadata from 360dialog (JSON with download URL)
    console.log(`[MEDIA] Requesting metadata for media ID: ${mediaId.substring(0, 20)}...`);
    ...
    // 360dialog returns JSON with a "url" field
    const metaData = await metaRes.json();
    const downloadUrl = metaData.url;
    const mimeType = metaData.mime_type || 'application/octet-stream';

    if (!downloadUrl) {
      console.error('[MEDIA] No URL in metadata:', JSON.stringify(metaData).substring(0, 200));
      throw new Error('No download URL in media metadata');
    }

    console.log(`[MEDIA] Got CDN URL: ${downloadUrl.substring(0, 60)}..., mime: ${mimeType}`);

    // Step 2: Download the actual file from WhatsApp CDN (no auth needed)
    ...
    console.error('[MEDIA] Download media error:', err.message);
    return null;
  }
}
