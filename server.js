...
async function generateAIResponse(studentMsg, phone) {
  ...
  // CRITICAL: When prospect asks about a specific course, inject an explicit instruction
  // to prevent the AI from dumping the full catalog
  const detectedCourseObj = getCourseByTitle(studentMsg);
  if (detectedCourseObj) {
    let courseFocus = '';
    if (isRE5Conversation || (detectedCourseObj && detectedCourseObj.title.toLowerCase().includes('re 5'))) {
      courseFocus = 'The prospect is asking about the RE 5 Regulatory Examination Preparation course. ONLY discuss RE 5. NEVER mention other courses like Banking NQF 5, Financial Markets NQF 6, Business Administration, HR Management, Entrepreneurship, or any other programme. Give RE 5 details and immediately push for registration. End with asking for their details to send the enrolment link.';
    } else if (isBankingNQF5Conversation || (detectedCourseObj && detectedCourseObj.title.toLowerCase().includes('banking nqf 5'))) {
      courseFocus = 'The prospect is asking about the National Certificate Banking NQF 5 course. ONLY discuss Banking NQF 5. NEVER mention other courses. Give Banking NQF 5 details and immediately push for registration. End with asking for their details to send the enrolment link.';
    } else if (isNQF6Conversation || (detectedCourseObj && detectedCourseObj.title.toLowerCase().includes('financial markets'))) {
      courseFocus = 'The prospect is asking about the National Certificate Financial Markets and Instruments NQF 6 course. ONLY discuss Financial Markets NQF 6. NEVER mention other courses. Give Financial Markets NQF 6 details and immediately push for registration. End with asking for their details to send the enrolment link.';
    } else if (isBusinessAdminConversation || (detectedCourseObj && detectedCourseObj.title.toLowerCase().includes('business administration'))) {
      courseFocus = 'The prospect is asking about the Online Advanced Business Administration course. ONLY discuss Business Administration. NEVER mention other courses. Give Business Administration details and immediately push for registration. End with asking for their details to send the enrolment link.';
    } else {
      courseFocus = 'The prospect is asking about the ' + detectedCourseObj.title + ' course. ONLY discuss this specific course. NEVER dump the full catalog or mention other programmes. Give this course\'s details and immediately push for registration.';
    }
    messages.push({ role: 'system', content: courseFocus });
  }
  ...
  // NUCLEAR FIX: When a specific course is detected, BYPASS OpenAI entirely.
  // OpenAI with temp 0.7 keeps ignoring our rules and dumping the full catalog.
  // Hardcoded fallback responses are 100% guaranteed to be correct.
  const detectedCourseForBypass = getCourseByTitle(studentMsg);
  if (detectedCourseForBypass) {
    const isRE5Bypass = detectedCourseForBypass.title.toLowerCase().includes('re 5');
    const isBankingBypass = detectedCourseForBypass.title.toLowerCase().includes('banking nqf 5');
    const isNQF6Bypass = detectedCourseForBypass.title.toLowerCase().includes('financial markets');
    const isBusinessAdminBypass = detectedCourseForBypass.title.toLowerCase().includes('business administration');
    
    if (isRE5Bypass || isBankingBypass || isNQF6Bypass || isBusinessAdminBypass) {
      console.log('[BYPASS] Specific course detected: ' + detectedCourseForBypass.title + ' — Skipping OpenAI, using hardcoded response.');
    ...
    case 'lead_info':
      if (leadInfo.fullName || leadInfo.email || leadInfo.altPhone || leadInfo.dateOfBirth) {
        ...
        if (missing.length === 0) {
          ...
          const isBankingNQF5Interest = courseInterest && (courseInterest.toLowerCase().includes('banking nqf 5') || courseInterest.toLowerCase().includes('banking') && courseInterest.toLowerCase().includes('nqf'));
          const isNQF6Interest = courseInterest && (courseInterest.toLowerCase().includes('financial markets') || courseInterest.toLowerCase().includes('nqf 6'));
          const isBusinessAdminInterest = courseInterest && (courseInterest.toLowerCase().includes('business administration') || courseInterest.toLowerCase().includes('business admin'));
          if (isRE5Interest) {
...
Which area interests you — Finance, Business & HR, or Healthcare? I can give you exact pricing and the payment plan for any specific course. 😊`;
    ...
    case 'courses':
      ...
      if (relevantCourse) {
        const isRE5Course = relevantCourse.title.toLowerCase().includes('re 5');
        const isBankingCourse = relevantCourse.title.toLowerCase().includes('banking nqf 5');
        const isNQF6Course = relevantCourse.title.toLowerCase().includes('financial markets');
        const isBusinessAdminCourse = relevantCourse.title.toLowerCase().includes('business administration');
...
Shall I send you the enrolment link right now? 😊`;
          ...
          response = `Great choice! The **${relevantCourse.title}** is a fantastic programme.
